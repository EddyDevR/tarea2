...
export default async (req, res) => {
    ...
    switch (url) {
        case '/':
            const filePathSource = path.join('views', 'index.html');
            console.log(filePathSource);
            if (req.url === '/') {
                fs.readFile(filePathSource, (err, data) => {
                    if (err) {
                        const filePathError = path.join('views', '404.html');
        ...
        case '/author':
            const filePathAuthor = path.join('views', 'author.html');
            console.log(filePathAuthor);
            if (req.url === '/author') {
                fs.readFile(filePathAuthor, (err, data) => {
                    if (err) {
                        const filePathError = path.join('views', '404.html');
                        fs.readFile(filePathError, (err, data) => {
                            if (err) {
